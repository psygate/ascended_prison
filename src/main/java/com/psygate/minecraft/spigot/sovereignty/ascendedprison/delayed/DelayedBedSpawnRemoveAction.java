 ...
 */
public class DelayedBedSpawnRemoveAction extends DelayedAction {
    private final static Logger LOG = AscendedPrison.getLogger(DelayedBedSpawnRemoveAction.class.getName());
    private String reason;

    public DelayedBedSpawnRemoveAction(UUID player, String reason) {
        super(player);
        this.reason = reason;
    }

    public DelayedBedSpawnRemoveAction(AscendedprisonDelayedActionsRecord rec) {
        super(rec.getPuuid());
    ...
    public void process(Event ev) {
        if (ev instanceof PlayerJoinEvent) {
            ...
            if (p != null && !p.isDead()) {
                LOG.info("Unsetting bed spawn: " + p.getName() + "(" + p.getUniqueId() + ")");
    ...
    public String toString() {
        return DelayedBedSpawnRemoveAction.class.getSimpleName() + "[" + getPlayer() + "]";
    }
}
