 ...
 */
public class DelayedStoreInventoryPlayer extends DelayedAction {
    private final static Logger LOG = AscendedPrison.getLogger(DelayedStoreInventoryPlayer.class.getName());
    private String reason;

    public DelayedStoreInventoryPlayer(UUID player, String reason) {
        super(player);
        this.reason = reason;
    }

    public DelayedStoreInventoryPlayer(AscendedprisonDelayedActionsRecord rec) {
        super(rec.getPuuid());
    ...
    public void process(Event ev) {
        if (ev instanceof PlayerJoinEvent || ev instanceof PlayerRespawnEvent || ev instanceof PlayerSpawnLocationEvent) {
            ...
            if (p != null && !p.isDead()) {
                LOG.info("Storing inventory " + p.getName() + "(" + p.getUniqueId() + ")");
    ...
    public String toString() {
        return DelayedStoreInventoryPlayer.class.getSimpleName() + "[" + getPlayer() + "]";
    }
}
