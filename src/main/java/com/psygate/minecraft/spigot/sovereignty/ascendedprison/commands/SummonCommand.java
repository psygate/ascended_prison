 ...
 */
public class SummonCommand extends NucleusPlayerCommand {
    ...
    protected void subOnCommand(Player player, Command command, String s, String[] strings) throws Exception {
        ...
        if (!prisonopt.isPresent()) {
            ...
            if (prisonopt.get().isSummoned()) {
                player.sendMessage(ChatColor.RED + "Already summoned.");
            } else {
                player.sendMessage(ChatColor.GREEN + "Summoning queued.");
                SummonManager.getInstance().summonPlayer(player, prisonopt.get());
            }
        }
    }
    ...
    protected String[] getName() {
        return new String[]{"summonprisoner"};
    }
}
